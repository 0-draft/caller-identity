...
誰も使うべきでないものから、2 年前には存在しなかったものまで 7 本。
...
| presigned URL | URL そのもの | 同じ署名をクエリ文字列に移したもの。それだけで bearer token になる。 |
| SigV4a | secret から導出した P-256 鍵ペア | 非対称署名。AWS は公開鍵側だけ持てばよくなる。 |
...
長い解説は [docs/](docs/README.ja.md) に置いてある。クレデンシャルの地図、SigV4 の全工程、
フェデレーションと繰り返される trust policy の間違い、presigned URL と SigV4a、そして
[検証ページ](docs/05-verification.ja.md) に各主張の検証方法と根拠が弱い箇所を記録してある。
